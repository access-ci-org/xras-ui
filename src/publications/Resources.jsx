...
export default function Resources() {
  ...
  const handleResourceToggle = (resourceId, checked) => {
    ...
    if (checked) {
      nextIds = current.includes(resourceId) ? current : [...current, resourceId];
    } else {
      nextIds = current.filter((id) => id !== resourceId);
    }
  ...
  const renderResourceOption = (resource, optionIndex) => (
    <div className="form-check" key={`resource_${resource.resource_id}`}>
      <input
        ...
        onChange={(e) =>
          handleResourceToggle(resource.resource_id, e.target.checked)
        }
      />
      <label
